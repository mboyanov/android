...
public int collectCrystals(Igloo[] crystals)
{
	int sum=0;
 ...
 */
	public int detectCollisions(Asteroid asteroid) {
		...
		RectF xa0 = new RectF(asteroid.left - 20, asteroid.up - 20, asteroid.right + 20,
				asteroid.down + 20);
		if (xa0.contains(this.x, this.y) == false)
			return 0;

		if (this.y < asteroid.up && this.x < asteroid.left) {
			float y1 = (this.y - asteroid.up);
			y1 = y1 * y1;
			float x1 = (this.x - asteroid.left);
			x1 = x1 * x1;
			if (x1 + y1 < 400)
				return 1;
		}

		RectF xa2 = new RectF(asteroid.left, asteroid.up - 20, asteroid.right, asteroid.up);
		if (xa2.contains(this.x, this.y)) {
			this.y = asteroid.up - 20;
			return 2;
		}
		...
		if (this.y < asteroid.up && this.x > asteroid.right) {
			float y1 = (this.y - asteroid.up);
			y1 = y1 * y1;
			float x1 = (this.x - asteroid.right);
			x1 = x1 * x1;
			if (x1 + y1 < 400)
				return 3;
		}
		RectF xa4 = new RectF(asteroid.left - 20, asteroid.up, asteroid.left, asteroid.down);
		if (xa4.contains(this.x, this.y))
			return 4;
		RectF xa5 = new RectF(asteroid.left, asteroid.up, asteroid.right, asteroid.down);
		if (xa5.contains(this.x, this.y))
			return 5;
		RectF xa6 = new RectF(asteroid.right, asteroid.up, asteroid.right + 20,
				asteroid.down);
		if (xa6.contains(this.x, this.y))
			return 6;

		if (this.y > asteroid.down && this.x < asteroid.left) {
			float y1 = (this.y - asteroid.down);
			y1 = y1 * y1;
			float x1 = (this.x - asteroid.left);
			x1 = x1 * x1;
			if (x1 + y1 < 400)
		...
		RectF xa8 = new RectF(asteroid.left, asteroid.down, asteroid.right,
				asteroid.down + 20);
	...
	public int detectCollisions(Asteroid[] asteroid) {
		for (int i = 0; i < 10; i++) {
			int j = this.detectCollisions(asteroid[i]);
			if (j != 0)
				return j;
		}
		return 0;
